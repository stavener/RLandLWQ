...
def generate_linear_data(n_samples, sigma, filename="lindata"):
    """
    Generate data (x, y) where x is uniformly sampled from [-1, 1],
    y = x + noise, noise ~ N(0, sigma^2).
...
def least_squares_fit(datafile="lindata", out_file="lsqdata"):
    """
    Read (x, y) data from file, compute least squares linear fit,
    evaluate at 100 points in [-1, 1], plot, and save to out_file.
    """
    data = np.loadtxt(datafile, skiprows=1)  # skip header
    x, y = data[:, 0], data[:, 1]
    # Fit y = a*x + b
...
def plot_all(datafile="lindata", lsqfile="lsqdata", mlpfile="MLPdata", out_file="all_fits.png"):
    """
    Plot the data, least squares fit, and MLP fit on the same graph.
    """
    data = np.loadtxt(datafile, skiprows=1)
    x, y = data[:, 0], data[:, 1]
